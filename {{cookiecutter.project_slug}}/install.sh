...
# Arg check
    if [ $# -eq 0 ]; then
        ...
        echo "Example:  ./install.sh /Oasis/{{cookiecutter.organization.replace(' ', '').lower()}}_{{cookiecutter.model_identifier.replace(' ', '').lower()}}/model_data/"
        exit 1
    fi
    export MODEL_DATA_ROOT=$1/$DATA_VER

if [[ ! -d ${MODEL_DATA_ROOT} ]]; then
    echo "Model data missing (${MODEL_DATA_ROOT}). Exiting."
    exit 1
fi
